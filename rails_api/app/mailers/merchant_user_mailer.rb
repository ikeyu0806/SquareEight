class MerchantUserMailer < ApplicationMailer
  ...
  def send_additional_user_verification_code(email, encode_email, verification_code)
    @email = email
    @verification_code = verification_code
    @url = ENV["FRONTEND_URL"] + '/merchant/invite/verification_code?email=' + encode_email
    mail(to: @email, subject: "【検証コード】SquareEight")
  end
