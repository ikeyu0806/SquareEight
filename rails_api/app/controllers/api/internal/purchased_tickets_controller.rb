class Api::Internal::PurchasedTicketsController < ApplicationController
  ...
  def index
    purchased_tickets = JSON.parse(current_end_user.purchased_tickets.to_json(methods: [:name, :display_expired_at]))
  ...
  def show
    purchased_ticket = JSON.parse(PurchasedTicket.find(params[:id]).to_json(methods: [:name, :display_expired_at]))
